 ...
 * along with the Arduino WaveBit Library.  If not, see
...
FAT16 or FAT32 file systems.  The WaveBit only supports short FAT 8.3 names.

WaveBit does not support MMC flash cards.
...
Also see the readme.txt file for instructions on installing WaveBit.

Advanced users may need to edit the WavePinDefs.h file.

WaveBit uses a slightly restricted form of short file names.
...
\section HowTo How to Format and Prepare SD Cards for WaveBit

WaveBit is optimized for contiguous files.  It will only play 16-bit
